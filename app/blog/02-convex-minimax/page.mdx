...
$f(\cdot, y) \colon X \to \R$ for each $y \in Y$ and $f(x, \cdot)$ is concave for each $x \in X$.
[^1]
...
\end{align*}
$$
Denote the optimal value of this convex optimization by $p^*$.
...
In convex optimization the situation where $p^* = d^*$, but I really think of it as just a minimax theorem.
To make the phrasing as a minimax theorem more clear, we can write the conclusion as a formula:
$$
...
1. $\mathcal{L}(x, \lambda)$ is a convex-_linear_ function, not just convex-concave.
...
[^1]: [LJJ20](http://proceedings.mlr.press/v125/lin20a/lin20a.pdf)
